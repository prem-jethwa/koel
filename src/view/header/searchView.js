...
class SearchView extends View {
  ...
  addHandlerSearchByArrowKey(handler) {
    this._searchInput.addEventListener('keyup', e => {
      if (e.key !== 'Enter') handler(this._searchInput.value, this._language.value);
  ...
  addhandlerSearchByKeyword(handler) {
    this._searchInput.addEventListener('keyup', e => {
      ...
      if (!keyword || e.key === 'Enter') return this._hideContent(this._displayResult);
  ...
  renderSearchResults(matchingSongs, val) {
    let html = matchingSongs
      .map((song, i) => {
        ...
        const titleMatch = song.title.replace(regex, `<span class="high-light" >${val}</span>`);
        const singerMatch = song.artist.replace(regex, `<span class="high-light" >${val}</span>`);
  ...
  renderBackToPlaylistBtn() {
    ...
    <div class="back-to-playlist_btn flex-container">
      <button><img src="icons/left-arrow.svg" alt="arrow"> Back To Playlist </button>
      <p> Search Results </p>
    </div>
    `;
